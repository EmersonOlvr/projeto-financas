	...
	public ModelAndView adicionarReceita(@ModelAttribute Receita receita, HttpServletRequest request) {
		try {
			...
			try {
				...
				this.logService.salvar(new Log(receita.getUsuario(), TiposLogs.CADASTRO_RECEITA, LocalDateTime.now(), request.getRemoteAddr()));
			} catch (Exception e) {
			...
			try {
				...
				this.logService.salvar(new Log(receita.getUsuario(), TiposLogs.ERRO_CADASTRO_RECEITA, LocalDateTime.now(), request.getRemoteAddr()));
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
